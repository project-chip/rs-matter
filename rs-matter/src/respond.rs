    ...
    B: BufferAccess<IMBuffer>,
{
    ...
    #[inline(always)]
    pub const fn new_default(data_model: &'d DataModel<'a, 'b, NS, NE, B, T>) -> Self
    where
        T: DataModelHandler,
    {
        Self::new(
...
pub struct DefaultResponder<'d, 'a, 'b, const NS: usize, const NE: usize, B, T>
where
    B: BufferAccess<IMBuffer>,
{
    responder: Responder<'a, DefaultExchangeHandler<'d, 'a, 'b, NS, NE, B, T>>,
    busy_responder: Responder<'a, BusyExchangeHandler>,
}

impl<'d, 'a, 'b, const NS: usize, const NE: usize, B, T> DefaultResponder<'d, 'a, 'b, NS, NE, B, T>
where
    B: BufferAccess<IMBuffer>,
    T: DataModelHandler,
{
    ...
    #[inline(always)]
    pub const fn new(data_model: &'d DataModel<'a, 'b, NS, NE, B, T>) -> Self {
        Self {
