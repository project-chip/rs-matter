...
//!
//! Compare with `WriteRequestMessageBuilder` in
...
use crate::im::{AttrPath, DataVersionFilter, EventFilter, EventPath, NodeId, ReadReqTag};
use crate::tlv::{TLVBuilderParent, TLVTag, TLVWrite, ToTLV};

/// Context tags for the fields of `AttributePathIB` (Matter Core spec
/// §10.6.2). Mirrored from `write_builder` — kept module-local since
/// the streaming builders are the only consumers. Promote to a
/// public enum if/when wider call sites need it.
#[repr(u8)]
enum AttrPathTag {
    _TagCompression = 0,
    Node = 1,
...
pub struct ReadRequestMessageBuilder<P, const F: usize> {
    p: P,
}

impl<P> ReadRequestMessageBuilder<P, 0>
where
    P: TLVBuilderParent,
{
    /// Begin a new `ReadRequestMessage` — opens an anonymous struct
    ...
    P: TLVBuilderParent,
{
    ...
    pub fn attr_requests(
        mut self,
    ) -> Result<AttrPathArrayBuilder<ReadRequestMessageBuilder<P, 1>>, Error> {
        self.p
            .writer()
            .start_array(&TLVTag::Context(ReadReqTag::AttrRequests as u8))?;
        Ok(AttrPathArrayBuilder {
            p: ReadRequestMessageBuilder { p: self.p },
        })
    }
    ...
    pub fn attr_requests_from(
        mut self,
        paths: &[AttrPath],
    ) -> Result<ReadRequestMessageBuilder<P, 1>, Error> {
        let w = self.p.writer();
        ...
        w.end_container()?;
        Ok(ReadRequestMessageBuilder { p: self.p })
    }
}
    ...
    P: TLVBuilderParent,
{
    ...
    pub fn event_requests_from(
        self,
        paths: &[EventPath],
    ) -> Result<ReadRequestMessageBuilder<P, 2>, Error> {
        ReadRequestMessageBuilder::<P, 1> { p: self.p }.event_requests_from(paths)
    }
}

impl<P> ReadRequestMessageBuilder<P, 1>
where
    P: TLVBuilderParent,
{
    ...
    P: TLVBuilderParent,
{
    pub fn event_filters_from(
        self,
        filters: &[EventFilter],
    ) -> Result<ReadRequestMessageBuilder<P, 3>, Error> {
        ReadRequestMessageBuilder::<P, 2> { p: self.p }.event_filters_from(filters)
    }
}
...
impl<P> ReadRequestMessageBuilder<P, 2>
where
    P: TLVBuilderParent,
{
    ...
    P: TLVBuilderParent,
{
    ...
    /// `EventFilters`.
    pub fn fabric_filtered(self, value: bool) -> Result<ReadRequestMessageBuilder<P, 4>, Error> {
        ReadRequestMessageBuilder::<P, 3> { p: self.p }.fabric_filtered(value)
    }
}
...
impl<P> ReadRequestMessageBuilder<P, 3>
where
    P: TLVBuilderParent,
{
    ...
    /// access to.
    pub fn fabric_filtered(
        mut self,
        value: bool,
    ) -> Result<ReadRequestMessageBuilder<P, 4>, Error> {
        self.p
            .writer()
...
// ---------------------------------------------------------------------
impl<P> ReadRequestMessageBuilder<P, 4>
where
    P: TLVBuilderParent,
{
    ...
    ) -> Result<ReadRequestMessageBuilder<P, 5>, Error> {
        let w = self.p.writer();
    ...
    P: TLVBuilderParent,
{
    ...
    pub fn end(self) -> Result<P, Error> {
        ReadRequestMessageBuilder::<P, 5> { p: self.p }.end()
    }
}

impl<P> ReadRequestMessageBuilder<P, 5>
where
    P: TLVBuilderParent,
{
...
impl<P, const F: usize> TLVBuilderParent for ReadRequestMessageBuilder<P, F>
where
    P: TLVBuilderParent,
{
    type Write = P::Write;
...
impl<P, const F: usize> core::fmt::Debug for ReadRequestMessageBuilder<P, F>
where
    P: core::fmt::Debug,
{
...
#[cfg(feature = "defmt")]
impl<P, const F: usize> defmt::Format for ReadRequestMessageBuilder<P, F>
where
    P: defmt::Format,
{
...
/// [`ReadRequestMessageBuilder::attr_requests`]; close with `.end()`
    ...
    P: TLVBuilderParent,
{
    ...
    pub fn push(mut self) -> Result<AttrPathBuilder<Self, 0>, Error> {
        self.p.writer().start_list(&TLVTag::Anonymous)?;
        Ok(AttrPathBuilder {
            p: self,
            _f: PhantomData,
        })
