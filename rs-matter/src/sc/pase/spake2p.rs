...
use crate::crypto::{
    as_canon, as_canon_mut, CanonEcPoint, CanonEcScalar, Crypto, Digest, EcPoint, EcScalar, Hash,
    HmacHash, Kdf, PbKdf, UInt, EC_CANON_POINT_LEN, EC_CANON_SCALAR_LEN, EC_POINT_ZEROED, HASH_LEN,
...
const MATTER_N_BIN: &CanonEcPoint = &[
...
impl<'a, C: Crypto> CryptoSpake2<'a, C> {
    ...
    pub fn set_w0_from_w0s(&mut self, w0s: &CanonEcScalar) -> Result<(), Error> {
        // From the Matter Spec,
        //         w0 = w0s mod p
        //   where p is the order of the curve
        const OPERAND: &CanonEcScalar = &[
    ...
    pub fn set_w1_from_w1s(&mut self, w1s: &CanonEcScalar) -> Result<(), Error> {
        // From the Matter Spec,
        //         w1 = w1s mod p
        //   where p is the order of the curve
        const OPERAND: &CanonEcScalar = &[
    ...
    pub(crate) fn set_w0(&mut self, w0: &CanonEcScalar) -> Result<(), Error> {
        self.w0 = Some(self.crypto.ec_scalar(w0)?);
        Ok(())
    }

    pub(crate) fn set_w1(&mut self, w1: &CanonEcScalar) -> Result<(), Error> {
    ...
    #[allow(non_snake_case)]
    #[allow(dead_code)]
    pub fn set_L(&mut self, l: &CanonEcPoint) -> Result<(), Error> {
        self.L = Some(self.crypto.ec_point(l)?);
        Ok(())
    }

    #[allow(non_snake_case)]
    pub fn set_L_from_w1s(&mut self, w1s: &CanonEcScalar) -> Result<(), Error> {
        // From the Matter spec,
        //        L = w1 * P
    ...
    pub fn get_TT_as_verifier(
        ...
        Self::add_to_tt(&mut TT, &point);
        // w0
        let scalar: &mut CanonEcScalar = unwrap!(as_canon_mut(&mut point[..EC_CANON_SCALAR_LEN]));
        unwrap!(self.w0.as_ref()).write_canon(scalar);
...
const VERIFIER_SIZE_BYTES: usize = EC_CANON_SCALAR_LEN + EC_CANON_POINT_LEN;
...
impl<'a, C: Crypto> Spake2P<'a, C> {
    ...
    pub(crate) fn start_verifier(&mut self, verifier: &VerifierData) -> Result<(), Error> {
        ...
        if let Some(pw) = verifier.password {
            ...
            if let Some(spake2) = &mut self.spake2 {
                spake2.set_w0(unwrap!(as_canon(&verifier.verifier[..EC_CANON_SCALAR_LEN])))?;
                spake2.set_L(unwrap!(as_canon(&verifier.verifier[EC_CANON_SCALAR_LEN..])))?;
            }
        }
    ...
    fn get_Ke_and_cAcB(
        ...
        let KaKe = TT;
        let Ka = &KaKe[..HASH_LEN / 2];
        let ke_internal = &KaKe[HASH_LEN / 2..];
...
#[cfg(test)]
mod tests {
    use crate::crypto::{test_crypto, Crypto, Digest, HASH_ZEROED, UINT384_CANON_LEN};
    ...
    pub struct RFCTestVector {
        pub w0: CanonEcScalar,
        pub w1: CanonEcScalar,
        pub x: CanonEcScalar,
        pub X: CanonEcPoint,
        pub y: CanonEcScalar,
        pub Y: CanonEcPoint,
