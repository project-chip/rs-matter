    ...
    ($source:expr) => {
        match $source {
            fut => {
                ...
                // If we run this code with const_ready, the binary size is smaller, i.e. future state machine better optimized
                // const is_ready : bool = $crate::dm::extract_ready_check($crate::dm::ReadyCheck::<false>);
                let is_ready : bool = $crate::dm::extract_ready_check((&&fut).get_check());
                if is_ready {
                    $crate::dm::MaybeReady(fut).get_ready()
                } else {
                    fut.await
                }
           }
...
pub struct MaybeReady<T>(pub T);

// Case 1: The specific type (Ready<T>)
impl<T> MaybeReady<core::future::Ready<T>> {
    #[inline(always)]
...
// Case 2: The fallback (Anything else that is a Future)
// We use a trait to provide the fallback to avoid naming conflicts
pub trait NotReadyFallback<T> {
    fn get_ready(self) -> T;
}

impl<T, F: core::future::Future<Output = T>> NotReadyFallback<T> for MaybeReady<F> {
    #[inline(always)]
    fn get_ready(self) -> T {
        unimplemented!()
    }
