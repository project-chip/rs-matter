...
impl NodeInfo {
    ...
    fn describe(&self) {
        ...
        for (id, device_types, clusters) in &self.endpoints {
            ...
            if in_pics_item {
                if let Some(n) = between(trimmed, "<itemNumber>", "</itemNumber>") {
                    ...
                    if let Some(name) = item.as_deref() {
                        ...
                        // those items.
                        if let Some(answer) =
                            answers.get(name).copied().or_else(|| self.resolve(name))
                        {
    ...
    fn derive_transports(&self, report: &mut Report) {
        let Some(cnet) = self.servers.get(&CNET_CLUSTER) else {
            return;
        };
    ...
    fn derive_node_wide(&self, report: &mut Report) {
        ...
        let serves = |id: u32| self.servers.contains_key(&id);
        ...
        set("MCORE.IDM.S", !self.servers.is_empty());
        let client = !self.clients.is_empty();
        ...
        let dlog_client = self.clients.contains(&DIAGNOSTIC_LOGS);
        ...
        let comm = &self.mdns.commissionable_txt;
        let oper = &self.mdns.operational_txt;
...
impl Report {
    ...
    fn apply_answers(&mut self, source: &str, answers: &BTreeMap<String, bool>) {
        let answered: Vec<(String, bool)> = self
            .left_items
            .iter()
            .filter_map(|item| answers.get(item).map(|v| (item.clone(), *v)))
            .collect();
        ...
        self.derived.extend(answered);
        self.left_items
            .retain(|left| !self.derived.contains_key(left));
    }
    ...
    fn ask_questions(&mut self, baseline: Option<&BTreeMap<String, bool>>) {
        ...
        for question in QUESTIONS {
            ...
            let applies = question
                .requires
                .iter()
                .all(|item| self.derived.get(*item).copied().unwrap_or(false));

            if !applies {
                for item in pending {
                    self.derived.insert(item.clone(), false);
                }
                continue;
            }
            ...
            for item in pending {
                self.derived.insert(item.clone(), answer);
            }
            asked += 1;
        }
    ...
    fn report_coverage(&self, path: &Path, baseline: &BTreeMap<String, bool>) {
        ...
        for item in baseline.keys() {
            if self.derived.contains_key(item) {
                checked += 1;
                continue;
            }
    ...
    fn lint_baseline(&self, path: &Path, baseline: &BTreeMap<String, bool>) {
        let mut conflicts: Vec<(&String, bool, bool)> = baseline
            .iter()
            .filter_map(|(item, claimed)| {
                let derived = *self.derived.get(item)?;
        ...
        self.report_omissions(baseline);
    }
    ...
    fn report_omissions(&self, baseline: &BTreeMap<String, bool>) {
        let mut missing: BTreeMap<&str, usize> = BTreeMap::new();

        for (item, derived) in &self.derived {
    ...
    fn write_flat(
        ...
        for (item, answer) in &self.derived {
            by_root
        ...
        if !self.left_items.is_empty() {
            ...
            let mut left = self.left_items.clone();
            left.sort();
            left.dedup();
        ...
        if let Some((path, map)) = baseline {
            let extra: Vec<_> = map
                .iter()
                .filter(|(k, _)| !self.derived.contains_key(*k) && !self.left_items.contains(k))
                .collect();
    ...
    fn fix_baseline(&self, path: &Path) -> anyhow::Result<usize> {
        let text =
...
pub fn run(
    ...
    for (name, xml) in &files {
        ...
        // decide.
        let _ = node.fill_template(xml, &BTreeMap::new(), &mut report);
    ...
    // see the same set.
    node.derive_transports(&mut total);
    node.derive_node_wide(&mut total);
    ...
    if let (Some(p), Some(map)) = (baseline, baseline_map.as_ref()) {
        total.report_coverage(p, map);
        total.lint_baseline(p, map);
    }
    ...
    if let Some(answers) = answers {
        ...
        total.apply_answers("answers file", &parse_flat(&text));
    }

    if let Some(map) = baseline_map.as_ref() {
        total.apply_answers("baseline", map);
    }

    if out.is_some() || pics.is_some() || fix {
        total.ask_questions(baseline_map.as_ref());
    }
    ...
    if let (Some(p), true) = (baseline, fix) {
        total.fix_baseline(p)?;
    }

    if let Some(pics) = pics {
        total.write_flat(pics, node_path, baseline.zip(baseline_map.as_ref()))?;
    }
    ...
    if let Some(out) = out {
        ...
        let filled: Vec<(String, String)> = retained
            ...
            .map(|(name, xml)| {
                ...
                (
                    (*name).to_string(),
                    node.fill_template(xml, &total.derived, &mut rendered),
                )
            })
            .collect();
