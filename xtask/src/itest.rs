...
/// a heating-only thermostat (`HEAT` alone) on EP1, metering its own heating
/// element through an Electrical Sensor device type on the same endpoint.
///
/// Deliberately absent:
...
pub(crate) const THERMOSTAT_TESTS: &[&str] = &[
    ...
    "TC_TSTAT_2_2",
    // Power Topology: with the `NODE` topology it serves no attributes at all,
    // so this is purely a check that the cluster is present and coherent.
    "TC_PWRTL_2_1",
    // Electrical Power Measurement / Electrical Energy Measurement attribute
    // reads. Both branch on the DUT's real feature map rather than on PICS, so
    // a single-phase AC, import-only device runs the applicable subset.
    "TC_EPM_2_1",
    "TC_EEM_2_1",
    // The changing-reading half of the same two clusters: these drive a fake
    // 1 kW load in through the GeneralDiagnostics `TestEventTrigger` command
    // and check that the readings move. `TC_EEM_2_3` and `TC_EEM_2_5` are the
    // exported-energy mirror of the last two and stay out: they ask for a
    // 3 kW *generator*, which needs the `EXPE` feature this device has no
    // hardware for.
    "TC_EPM_2_2",
    "TC_EEM_2_2",
    "TC_EEM_2_4",
    // The Thermostat device type (`0x0301`) is new with this driver, and the
    // Electrical Sensor (`0x0510`) shares its endpoint: check both mandatory
    // cluster sets and that the revision claims are honest.
    "TC_DeviceConformance",
];
...
impl ITests {
    ...
    fn needs_target_pics(test_name: &str) -> bool {
        matches!(
            ...
            | "TC_TSTAT_2_2"
            // `TC_EPM_2_1` and `TC_EEM_2_1` declare a top-level `pics_*` gate
            // (`["EPM.S"]` / `["EEM.S"]`), so with no `--PICS` the runner skips
            // the whole test - and `--fail-on-skipped` then fails the run.
            // `TC_EEM_2_1` additionally gates its per-attribute steps on
            // `EEM.S.A0001`..`A0005`.
            | "TC_EPM_2_1"
            | "TC_EEM_2_1"
