...
//!
//! The network itself is [`MockNetCtl`], exactly as for `wireless_tests` - so no
//! radio is involved on either side of the link. The Bluetooth adapter is real
//! as far as `rs-matter` is concerned, but it can equally be one published by
//! `bluezoo`, which is what makes this runnable on a machine with no Bluetooth
//! hardware at all.
//!
//! Arguments:
...
//!   equally be pointed at real hardware or at `bluezoo`.
...
/// The Node meta-data for the device.
