...
//! thermostat's setpoints, a heating element's energy counter - because several
//! tests restart the DUT mid-run and then assert on what came back.
//!
...
pub const HEATING_ELEMENT_ENERGY_KEY: u16 = VENDOR_KEYS_START + 1;
...
/// `FnMatcher` expressions, as `ThermostatDeviceLogic::CLUSTER` - so this narrows
